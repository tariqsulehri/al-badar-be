...
const syncCitiesFromSlides = async () => {
  const names = await Slide.distinct("city", {
    city: { $nin: [null, ""] },
  });
...
exports.listCitiesForSelect = async (req, res) => {
  try {
    await syncCitiesFromSlides();

    const cities = await City.find({ name: { $nin: [null, ""] } })
      .sort({ name: 1 })
      .select({ _id: 0, name: 1 })
      .lean();

    const resp = cities.map((item) => ({
      label: item.name,
      value: item.name,
    }));
