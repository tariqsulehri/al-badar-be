...
const syncProvencesFromSlides = async () => {
  const names = await Slide.distinct("provence", {
    provence: { $nin: [null, ""] },
  });
...
exports.listProvencesForSelect = async (req, res) => {
  try {
    await syncProvencesFromSlides();

    const provences = await Provence.find({ name: { $nin: [null, ""] } })
      .sort({ name: 1 })
      .select({ _id: 0, name: 1 })
      .lean();

    const resp = provences.map((item) => ({
      label: item.name,
      value: item.name,
    }));
