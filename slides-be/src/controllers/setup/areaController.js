...
const syncAreasFromSlides = async () => {
  const names = await Slide.distinct("area", {
    area: { $nin: [null, ""] },
  });
...
exports.listAreasForSelect = async (req, res) => {
  try {
    await syncAreasFromSlides();

    const areas = await Area.find({ name: { $nin: [null, ""] } })
      .sort({ name: 1 })
      .select({ _id: 0, name: 1 })
      .lean();

    const resp = areas.map((item) => ({
      label: item.name,
      value: item.name,
    }));
