...
const syncSubAreasFromSlides = async () => {
  const names = await Slide.distinct("subArea", {
    subArea: { $nin: [null, ""] },
  });
...
exports.listSubAreaForSelect = async (req, res) => {
  try {
    await syncSubAreasFromSlides();

    const subAreas = await SubArea.find({ name: { $nin: [null, ""] } })
      .sort({ name: 1 })
      .select({ _id: 0, name: 1 })
      .lean();

    const resp = subAreas.map((item) => ({
      label: item.name,
      value: item.name,
    }));
