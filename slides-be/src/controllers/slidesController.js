...
exports.uploadImage = async (req, res) => {
  upload(req, res, (err) => {
    ...
    const filePath = `${baseUrl}/uploads/slides/images/${req.file.filename}`;
    res.status(200).json({
...
exports.listSlides = async (req, res) => {
  try {
    const { pageNo = 1, pageSize = 10, searchBy = "name", searchText = "", filters = "{}" } = req.query;
    let parsedFilters = {};
    ...
    console.error("Error in listSlides:", error.message);
